...
const DEFAULT_GLOSSARY = [
  ...
  {
    "id": 13,
    "en": "Bleak Falls Barrow",
    "es": "Túmulo de las Cataratas Lúgubres",
    "cat": "Lugares"
  },
  {
    "id": 14,
  ...
  {
    "id": 15,
    "en": "Throat of the World",
    "es": "Garganta del Mundo",
    "cat": "Lugares"
  },
  {
    "id": 16,
  ...
  {
    "id": 28,
    "en": "Unrelenting Force",
    "es": "Fuerza implacable",
    "cat": "Habilidades"
  },
  {
    "id": 29,
  ...
  {
    "id": 33,
    "en": "College of Winterhold",
    "es": "Colegio de Hibernalia",
    "cat": "Lugares"
  },
  {
    "id": 34,
  ...
  {
    "id": 64,
    "en": "Dragonplate Armor",
    "es": "Armadura de placas de dragón",
    "cat": "Armaduras"
  },
  {
    "id": 65,
    "en": "Daedric Armor",
    "es": "Armadura daédrica",
    "cat": "Armaduras"
  },
  ...
  {
    "id": 67,
    "en": "Dragon Priest Mask",
    "es": "Máscara de sacerdote dragón",
    "cat": "Items"
  },
...
export default function App() {
  ...
  const saveGlossary = (g) => { 
    setGlossaryState(g); 
    localStorage.setItem("glossary_v2", JSON.stringify(g));
  };
  const saveConfig = (c) => { 
    setConfigState(c); 
    localStorage.setItem("config_v1", JSON.stringify(c));
  };
  const savePresets = (p) => { 
    setPresets(p); 
    localStorage.setItem("presets_v1", JSON.stringify(p));
  };
  // Sync resize
  const onMouseDown = (e) => {
    resizingRef.current = true;
    startYRef.current = e.clientY;
    startHRef.current = taHeight;
    e.preventDefault();
  };
  useEffect(() => {
    const onMove = (e) => { if (!resizingRef.current) return; const dy = e.clientY - startYRef.current; setTaHeight(Math.max(120, startHRef.current + dy)); };
...
const callAPI = async (userMsg) => {
  ...
  if (config.engine === "google") {
    return result.google?.[0]?.map(x => x[0]).join("") || "";
  }
  ...
  const handleAddEntry = async () => {
    ...
    const entry = { id:Date.now(), en:newEn.trim(), es:newEs.trim(), cat };
    saveGlossary([...glossary, entry]);
  ...
  const handleCopy = () => {
    const ta = exportTextareaRef.current; if (!ta) return;
    ta.select(); ta.setSelectionRange(0,99999); document.execCommand("copy"); window.getSelection()?.removeAllRanges();
    setCopied(true); setTimeout(()=>setCopied(false),2000);
  };
  const importFromText = () => {
    try {
      const parsed = JSON.parse(importText);
      const arr = Array.isArray(parsed) ? parsed : Object.entries(parsed).map(([en,es],i)=>({id:i+1,en,es,cat:"Otros"}));
      saveGlossary(arr); setImportText(""); setImportError(""); setShowImport(false);
    } catch { setImportError("JSON inválido."); }
  };
  const importGlossary = (e) => {
    const file = e.target.files[0]; if (!file) return;
    const r = new FileReader();
    r.onload = (ev) => { try { const p=JSON.parse(ev.target.result); saveGlossary(Array.isArray(p)?p:Object.entries(p).map(([en,es],i)=>({id:i+1,en,es,cat:"Otros"}))); } catch { setImportError("Archivo inválido."); } };
  ...
  const addPreset = () => {
    ...
    const id = Date.now();
    const updated = [...presets, { id, name:newPresetName.trim(), text:newPresetText.trim() }];
    savePresets(updated); setSelectedPreset(id); setNewPresetName(""); setNewPresetText("");
  };
        ...
        <div style={{ background:"#1a1a2e", borderRadius:"0 8px 8px 8px", padding:20 }}>
          ...
          {tab==="translator" && (
            <div>
              ...
              {rateLimitMsg && (
                <div style={{ background:"#2a1a0a", border:"1px solid #e8a020", borderRadius:8, padding:"12px 16px", marginBottom:14, display:"flex", gap:10, alignItems:"flex-start" }}>
                  ...
                  <div>
                    ...
                    <button onClick={()=>setTab("settings")} style={{ marginTop:6, padding:"3px 10px", background:"#7c4a1e", color:"#e8c87a", border:"none", borderRadius:4, cursor:"pointer", fontSize:12 }}>
                      Cambiar motor →
                    </button>
                  </div>
                </div>
              ...
              {/* Textareas sincronizadas */}
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                <div>
                  <label style={{ color:"#e8c87a", fontSize:13 }}>Texto en inglés</label>
                  <textarea value={inputText} onChange={e=>setInputText(e.target.value)} placeholder="Pega aquí el texto extraído con ESP Translate..."
                    style={{ width:"100%", height:taHeight, marginTop:6, background:"#0d0d1a", color:"#ddd", border:"1px solid #3a3a5c", borderRadius:6, padding:10, fontSize:13, resize:"none", boxSizing:"border-box" }} />
                </div>
                <div>
                  <label style={{ color:"#e8c87a", fontSize:13 }}>Traducción al español latino</label>
                  <textarea value={outputText} readOnly placeholder="La traducción aparecerá aquí..."
                    style={{ width:"100%", height:taHeight, marginTop:6, background:"#0d0d1a", color:"#8ddb8d", border:"1px solid #3a3a5c", borderRadius:6, padding:10, fontSize:13, resize:"none", boxSizing:"border-box" }} />
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ background:"#0d0d1a", borderRadius:8, padding:16, border:"1px solid #3a3a5c" }}>
                {config.engine==="claude" && (<>
                  <p style={{ color:"#e8c87a", fontSize:13, marginBottom:12 }}>Claude (Anthropic) — se usa la key del entorno si no se especifica</p>
                ...
                {config.engine==="deepl" && (<>
                  ...
                  <p style={{ color:"#555", fontSize:11 }}>Nota: DeepL no usa el prompt del sistema, solo traduce el texto directamente.</p>
                </>)}
              ...
              <p style={{ color:"#555", fontSize:11, marginTop:10 }}>⚠️ Las API Keys se guardan en el storage local del artefacto. No las compartas con nadie.</p>
            </div>
          )}
